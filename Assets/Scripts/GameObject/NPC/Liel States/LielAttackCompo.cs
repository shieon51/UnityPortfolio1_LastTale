...
public class LielAttackCompo : MonoBehaviour
{
    ...
    public void AE_StartDash1()
    {
        ...
        // AddForce(밀기)를 버리고, 목표 속도(Velocity)를 즉시 덮어씌움.
        // 가속도가 붙는 시간이 생략되어 1프레임만에 최고 속도로 튀어나감.
        rb.linearVelocity = new Vector2(dir * dash1.burstSpeed, 0f);

        Debug.Log("[Liel] 마나 소모 및 빛의 속도로 돌진 시작!");
    }

    // 2. [ AE ] 돌진 중 끼익-멈추기 시작 타이밍 (선형 마찰력)
    public void AE_StartSlideStop1()
    {
        // 마찰력(Drag)을 팍 올려서 선형적으로 끼이익 멈추는 느낌 구현
    ...
    // =====================================================================
    // [공격 판정 코루틴] (PlayerAttack.cs 구조 복사)
    // =====================================================================
    private IEnumerator ActiveHitboxRoutine(HitboxSettings settings)
    {
        ...
        // 공격 시작하는 순간 '부모(몸통)' 위치를 기준으로 고정점 1번만 계산
    ...
    private void OnDrawGizmos()
    {
        ...
        if (isDrawingHitbox)
        {
            ...
            // 매 프레임 위치 재계산 안 함. 아까 코루틴에서 찍어둔 고정 좌표(lastHitboxCenter)를 그대로 그림.
            Gizmos.DrawCube(lastHitboxCenter, hitbox1.size);
        }
    }

    // 게임 실행 전 에디터에서 히트박스 미리 확인 가능
